...
module.exports = {
  ...
  getApps: async () => {
    let entry = await strapi.query('ZaloOA', 'zalo').find({});

    return { ok: true, data: entry };
  },

  addZaloOA: async (data) => {
    try {
      ...
      let entry = await strapi.query('ZaloOA', 'zalo').create(json);
  ...
  webhook: async (data) => {
    ...
    if (event_name === 'follow') {
      try {
        ...
        let entry = await strapi.query('ZaloOA', pluginName).findOne({
          app_id: data.app_id
        }, []);
  ...
  handleMessage: async (app_id, user_id, message, timestamp) => {
    let entry = await strapi.query('ZaloOA', pluginName).findOne({
      app_id: app_id
    }, []);
    ...
    let rule = await strapi.query('ZaloMessageRule', pluginName).findOne({
      rule: pattern,
      zalo_oa: entry.id
    }, []);
    ...
    if (rule.activity && rule.activity !== "103") {
      let follower = await strapi.query('ZaloFollower', pluginName).findOne({
        user_id: user_id,
        zalo_oa: entry.id
      }, []);
  ...
  handleRead: async (data) => {
    ...
    let reports = await strapi.query('ZaloReport', pluginName).find({
  ...
  addContact: async (user_id, timestamp, mobile, token, ZOAId) => {

    try {
      let entry = await strapi.query('ZaloFollower', pluginName).findOne({
        user_id: user_id
      }, []);
      ...
      if (!entry) {
        ...
        entry = await strapi.query('ZaloFollower', pluginName).update({
          id: entry.id
        }, {
  ...
  prepareSync: async (id) => {
    let entry = await strapi.query('ZaloOA', 'zalo').findOne({
      id: id
    }, []);
  ...
  syncFollowers: async (offset, count, token, ZOAId) => {
    try {
      const config = {
        method: 'get',
  ...
  getZaloFollower: async (user_id, timestamp, token, ZOAId) => {
    try {
      const config = {
        method: 'get',
      ...
      if (res.data.error === 0 && res.data.data) {
        ...
        //upsert to db
        if (!entry) {
          entry = await strapi.query('ZaloFollower', pluginName).create(data);
        } else {
          entry = await strapi.query('ZaloFollower', pluginName).update({
            user_id: user_id
          }, data);
  ...
  updateUnfollower: async (user_id, timestamp, token, ZOAId) => {
    try {
      let entry = await strapi.query('ZaloFollower', pluginName).findOne({
        user_id: user_id
      });
  ...
  sendZaloMessage: async (data) => {
    ...
    if (message) {
      ...
      let msgTemplate = await strapi.query('ZaloMessageTemplate', pluginName).findOne({
        id: template
      }, []);
    ...
    try {
      ...
      if (entry.mobile_number && res.data.error === 0) {
        //store to db
        let report = await strapi.query('ZaloReport', pluginName).create({
          user_id: ZALO_OA_ID,
